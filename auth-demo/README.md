...
> 1. Cài package: `dotnet add package Npgsql.EntityFrameworkCore.PostgreSQL`
> 2. Trong `Program.cs` đổi `UseSqlServer(...)` thành `UseNpgsql(...)`
> 3. Đổi connection string trong `appsettings.json`:
>    `"Host=localhost;Database=AuthDemoDB;Username=postgres;Password=yourpassword"`
